...
class RoleController extends Controller
{
    ...
    function AllUsersList(Request $request)
    {
        ...
        if ($emal_mob) {
            $query->where('user_account.email', 'LIKE', '%' . $emal_mob . '%')
                ->orWhere('user_account.mobno', 'LIKE', '%' . $emal_mob . '%');
        }
        if ($uname) {
    ...
    function AdmUserViewEdit(Request $request)
    {
        ...
        $query = UserAccount::select('user_account.*','roles.role_name')
        ->leftJoin('roles', 'user_account.role_id', 'roles.id')->where('user_account.id', $id);
        $alluserdetails = $query->first();
        //echo $lastRegId = $query->toSql();exit;
        $roles     = DB::table('roles')->where('id',$alluserdetails->role_id)->get();
    ...
    function AdmUpdateUserDetails(Request $request)
        {
            ...
            if($request->eroleid==2 || $request->eroleid==9)
                {
                    $userselrid     = DB::table('seller_details')->select('id')->where('user_id', $useridhid)->get();
            ...
            else if($request->eroleid==3)
                {
                    $affiliateid     = DB::table('affiliate')->select('id')->where('user_id', $useridhid)->get();
        ...
        function AdmuserDeletePage(Request $request)
        {
            ...
            $UsrroleID     = DB::table('user_account')->select('role_id')->where('id', $userid)->get();
            foreach($UsrroleID as $rolid)
            {
                $roles=$rolid->role_id;
            }
            if($roles==2 || $roles==9){
                $sellerID     = DB::table('seller_details')->select('id')->where('user_id', $userid)->get();
                ...
                $affiliateID     = DB::table('affiliate')->select('id')->where('user_id', $userid)->get();
        ...
        function AdmUserMenuViewPage(Request $request)
        {
            ...
            $query = DB::table('menu_masters')->orderBy('menu_level_1')->orderBy('menu_level_2')->orderBy('menu_level_3')->get();
            $allmenus = $query;
            //echo $lastRegId = $query->toSql();exit;
        ...
        function AdmAddUserPageMenu(Request $request)
        {
            ...
            for ($i = 0; $i < $mencount; $i++) {
                if (!empty($menuval[$i])) {
                    $userroleID = DB::table('user_account')->select('role_id')->where('id', $user_id)->get();
            ...
            for ($k = 0; $k < $nums; $k++) {
                if (!empty($menus[$k])) {
                    $updteUserPage = UserPage::where('user_id', $user_id)->where('menu_id', $menus[$k])->update(['privilage' => 'A']);
                }
            }
        ...
        function AdmgetUserMenu(Request $request)
        {
            ...
            $user_id      = $request->input('user_id');
            $menuIds = DB::table('user_pages')->where('user_id', $user_id)->pluck('menu_id')->toArray();
        ...
        function AdmgetPrivilageMenu(Request $request)
        {
            ...
            $user_id      = $request->input('user_id');
            $menuIds = DB::table('user_pages')->where('user_id', $user_id)->where('privilage', 'A')->pluck('menu_id')->toArray();
            $menuIdsString = implode(',', $menuIds);
            echo "," . $menuIdsString . ",";
        ...
        function AdmAddRolePageMenu(Request $request)
        {
            ...
            $nums = count($menus);
            $menuIds = DB::table('role_pages')->where('role_id', $role_id)->pluck('menu_id')->toArray();
            $md = $menuIds;
            $m = count($md) - 1;
            DB::table('role_pages')->where('role_id', $role_id)->delete();
            $arr = explode(',', $values);
            $n = count($arr) - 1;
            ...
            $flag = 0;
            $userIds = DB::table('user_pages')->where('user_role', $role_id)->distinct()->pluck('user_id')->toArray();
            $user = $userIds;
            $u = count($user) - 1;
        ...
        function AdmgetrolePrivilage(Request $request)
        {
            ...
            $menuIds = DB::table('role_pages')->where('role_id', $roleid)->where('privilage', 'A')->pluck('menu_id')->toArray();
            $menuIdsString = implode(',', $menuIds);
            echo "," . $menuIdsString . ",";
        ...
        function AdmgetUserRoleMenu(Request $request)
        {
            ...
            $user_id = $request->input('user_id');
            $userRoles = DB::table('user_account')->select('role_id')->where('id', $user_id)->pluck('role_id')->toArray();
        ...
        public function AdmAddUserRolePageMenu(Request $request)
            {
                ...
                DB::table('user_pages')->where('user_id', $user_id)->delete();
                   if (!empty($rolesArray)) {
