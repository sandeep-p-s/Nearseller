...
class AdminController extends Controller
{
    ...
    function AdmsellerRegisterationPage(Request $request)
    {
        ...
        if ($submt > 0) {
            ...
            $sellerDetail->shop_name = ucwords($request->input('s_name'));
            $sellerDetail->owner_name = ucwords($request->input('s_ownername'));
            ...
            $sellerDetail->locality = ucwords($request->input('s_locality'));
            $sellerDetail->village = ucwords($request->input('s_villagetown'));
            ...
            $sellerDetail->direct_affiliate = ucwords($request->input('directafflte'));
            $sellerDetail->second_affiliate = ucwords($request->input('secondafflte'));
            $sellerDetail->shop_coordinator = ucwords($request->input('coordinater'));
    ...
    function AdmsellerUpdatePage(Request $request)
    {
        ...
        $sellerDetail->owner_name = ucwords($request->input('es_ownername')); //}
        ...
        $sellerDetail->locality = ucwords($request->input('es_locality'));
        $sellerDetail->village = ucwords($request->input('es_villagetown'));
        ...
        $user->name = ucwords($request->es_name);
    ...
    function ShopApprovalm($typeid)
    {
        ...
        $structuredMenu = MenuMaster::UserPageMenu($userId);
        $allsectdetails = MenuMaster::AllSecrtorDetails($userId, $roleid);
        return view('admin.shop_approval_m', compact('userdetails', 'userRole', 'loggeduser', 'shoporservice', 'typeid', 'structuredMenu', 'allsectdetails','selrdetails'));
    }

    function AllShopsListm(Request $request)
    {
        $userRole = session('user_role');
        $roleid = session('roleid');
        ...
        return view('admin.shop_dets_m', compact('sellerDetails', 'sellerCount', 'countries', 'business', 'shoporservice', 'typeid','shopservicecategory','shopservice','executives','shopavailable','activecounts'));
    }
